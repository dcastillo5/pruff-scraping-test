...
## Deploy frontend
To deploy your front end website to your AWS account:
1. Create distribution for deployment
```
npm run build
```

2.  Sync distribution with S3
```
aws s3 sync <dist_folder> s3://<your-bucket-name>
```
...
```bash
pruff-scraping$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pruff-scraping$ sam local start-api
pruff-scraping$ curl http://localhost:3000/
```
...
```bash
pruff-scraping$ sam logs -n HelloWorldFunction --stack-name pruff-scraping --tail
```
...
```bash
sam delete --stack-name pruff-scraping
```
